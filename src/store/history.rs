...
impl <'repo> FileHistoryIterator<'repo> {
    pub fn new<T: ToString>(repository: &'repo Repository, filename: T) -> anyhow::Result<FileHistoryIterator<'repo>> {
...
fn diff_to_string(diff: &Diff) -> String {
  ...
  diff.print(DiffFormat::Patch, |_delta, _hunk, line| {
      match line.origin_value() {
          DiffLineType::Context | DiffLineType::Addition | DiffLineType::Deletion => write!(result, "{}", line.origin()).unwrap(),
          _ => {}
      };
